...
C       Added logic to cycle to next report when ONE OR MORE SUBSETS READ 
C       DO NOT HAVE AN INTERNAL DATE SKIPPING TO NEXT REPORT and print an 
C       error message.
C 2024-03-20 M. Weiss --
C       1. Added call to the new routine bufr_restrict.f. This code
C          reads a list of restricted BUFR tank from the external
C          RSTPROD_BUFR_TANK.TBL file.
...
ccccccCALL W3TAGB('BUFR_TRANJB',2021,0337,1200,'NP22')
     ...
     $ SATINGEST_FIX_DIR)
      ...
      CALL GET_ENVIRONMENT_VARIABLE('rtank_tbl',RTANK_TBL)
...
C  Read IN THE LOOK-UP TABLE LISTING ALL BUFR TANKS REQUIRING
C   RESTRICTED ACCESS. READ INTO ARRAYS THAT ARE PASSED INTO TYPTIM
C   VIA COMMON BLOCK.
C  ------------------------------------------------------------------

      CALL BUFR_RESTRICT (RTANK_TBL, IRET)
